...
import type { DispatchOrder, FacilityRow } from '@/lib/district-detail';
...
export default function TransferMap({
  ...
  const project = useMemo(() => {
    ...
    return (lon: number, lat: number): [number, number] => {
      ...
      // A degenerate extent (every facility on one coordinate) makes d3 return
      // a non-finite scale. Falling back to the centre keeps the SVG valid
  ...
  return (
    <div className="relative">
      <svg
        ...
        role="img"
        aria-label={`Dispatch map: ${count(facilities.length)} facilities and ${count(orders.length)} recommended transfers`}
      >
