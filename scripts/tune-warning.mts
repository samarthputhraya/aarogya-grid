...
/**
 * THE TUNING PANEL: 128 districts, drawn from every state, fixed by the seed.
 *
 * The experiment was designed on a 128-district grid -- 42 surged per round, a
 * third of the panel, against 86 clean controls -- and its precision figure is a
 * property of that base rate. Run over all 769 districts with the same 42
 * surges, a third becomes a twentieth, precision falls for reasons that have
 * nothing to do with the rule, and every BigQuery round costs six times as much
 * to say so. So the rule is tuned on a panel the same size as the grid it was
 * designed on, and applied nationally.
 *
 * The panel is not the old 128. That was sixteen states chosen for where supply
 * failure is worst, which is a biased sample of Indian districts for a
 * detector. This one is a seeded draw that takes each state's share of the
 * country's districts, at least one each, so all thirty-six are represented.
 */
const PANEL_SIZE = 128;
function tuningPanel(codes: { code: string; stateCode: string }[]): Set<string> {
  const byState = new Map<string, string[]>();
  for (const c of codes) byState.set(c.stateCode, [...(byState.get(c.stateCode) ?? []), c.code]);
  const rank = (code: string) => createRng(hashSeed(SEED, 'tuning-panel', code)).next();
  // One seat per state first, then the rest in proportion to the districts each
  // state holds beyond its first, largest remainders taking the seats the floors
  // leave over -- so the panel is exactly PANEL_SIZE and every state is in it.
  const free = PANEL_SIZE - byState.size;
  const beyond = codes.length - byState.size;
  const quota = new Map<string, number>();
  const remainders: { state: string; r: number }[] = [];
  for (const [state, list] of byState) {
    const exact = ((list.length - 1) / beyond) * free;
    quota.set(state, 1 + Math.floor(exact));
    remainders.push({ state, r: exact % 1 });
  }
  remainders.sort((a, b) => b.r - a.r || a.state.localeCompare(b.state));
  let seats = PANEL_SIZE - [...quota.values()].reduce((a, b) => a + b, 0);
  for (let i = 0; seats > 0; i++, seats--) {
    const s = remainders[i % remainders.length].state;
    quota.set(s, Math.min(byState.get(s)!.length, quota.get(s)! + 1));
  }
  const panel = new Set<string>();
  for (const [state, list] of byState) {
    for (const code of [...list].sort((a, b) => rank(a) - rank(b)).slice(0, quota.get(state))) panel.add(code);
  }
  return panel;
}

const footfallAll = JSON.parse(
  readFileSync(resolve(root, 'src/data/footfall-district-daily.json'), 'utf8'),
) as FootfallArtefact;
const PANEL = tuningPanel(footfallAll.districts);
const footfall: FootfallArtefact = { ...footfallAll, districts: footfallAll.districts.filter((d) => PANEL.has(d.code)) };
const demandAll = JSON.parse(
  readFileSync(resolve(root, 'src/data/demand-district-daily.json'), 'utf8'),
) as DemandArtefact;
const demand: DemandArtefact = { ...demandAll, series: demandAll.series.filter((s) => PANEL.has(s.districtCode)) };
...
const artefact = {
  ...
  panel: {
    size: districtCodes.length,
    states: new Set(footfall.districts.map((d) => d.stateCode)).size,
    modelledDistricts: footfallAll.districts.length,
    method: 'one district per state, the rest in proportion to each state\'s districts, seeded draw within state',
  },
...
function renderMarkdown(): string {
  ...
  lines.push('');
  lines.push(
    'Run ' + artefact.at + ' over a ' + districtCodes.length + '-district panel drawn from all ' +
      new Set(footfall.districts.map((d) => d.stateCode)).size + ' states and union territories (of ' +
      footfallAll.districts.length + ' modelled districts; see the panel note in the script), ' + ROUNDS +
  ...
  if (chosen) {
    ...
    lines.push('');
    /*
     * Generated, not typed. This paragraph used to name the chosen rule's
     * precision in words and quote two tighter rules by their figures; a re-run
     * over a different panel moves all of them, and a sentence nobody re-derives
     * is exactly the stale claim the drift guard exists to catch.
     */
    const tighter = evaluations
      .filter((e) => e.source === chosen.source && !meetsGate(e) && e.precision > chosen.precision)
      .filter((e) => {
        const two2 = e.byMultiplier.find((m) => m.multiplier === 2.0);
        // Misses the gate ONLY on lead time.
        return (two2?.rate ?? 0) >= GATE.detection && e.falseAlarmsPerDistrictWeek <= GATE.falseAlarms;
      })
      .sort((a, b) => b.precision - a.precision)
      .slice(0, 2);
    lines.push(
      '**' + pct(chosen.precision) + ' precision' +
        (chosen.precision < 0.5 ? ' is not a good number, and it is the honest one.**' : ' is published as measured.**') +
        ' The gate this project set itself -- from the design note, before any of this was measured -- constrains',
    );
    lines.push('detection, lead time and the false alarm rate, and says nothing about precision.');
    if (tighter.length > 0) {
      lines.push(
        'On this evidence it should have: ' +
          (tighter.length === 1 ? 'a tighter rule' : 'the tighter rules') + ' ' +
          tighter.map((e) => 'k=' + e.rule.k + ' e=' + e.rule.e).join(' and ') + ' reach ' +
          tighter.map((e) => pct(e.precision)).join(' and ') + ' precision for ' +
          tighter.map((e) => pct(e.byMultiplier.find((m) => m.multiplier === 2.0)?.rate ?? 0)).join(' and ') +
          ' detection, and miss the gate only on the ' + GATE.leadDays + '-day lead.',
      );
    }
    lines.push(
